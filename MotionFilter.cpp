...
void MotionFilter::process(cv::Mat const& image)
{
  ...
  else
  {
    ...
    if (workingSize.width != this->imageSize.width || workingSize.height != this->imageSize.height)
    {
      this->DeleteTemporaryStorage();
    ...
    if (this->difference.empty())
    {
      ...
      this->text_size =
          cv::getTextSize(timestring.toLatin1().data(), CV_FONT_HERSHEY_PLAIN, 1.0, 1, NULL);
    }
    ...
    if (1)
    {
      ...
      {
      case 0: shownImage = currentFrame; break;
      case 1: shownImage = this->movingAverage; break;
      case 2: shownImage = this->thresholdImage; break;
      default:
      case 3:
        /* dst = src1 * alpha + src2 * beta + gamma */
        cv::addWeighted(currentFrame, this->blendRatio, this->blendImage, 1.0 - this->blendRatio,
            0.0, this->blendImage);
      ...
      case 4:
        ...
        shownImage = this->blendImage;
        break;
      case 5: shownImage = this->noiseImage; break;
      }
    }
...
void MotionFilter::updateNoiseMap(cv::Mat const& image, double noiseblend)
{
  ...
  if (cnt == 0)
  {
    cv::rectangle(this->noiseImage, cvPoint(0, 0), this->noiseImage.size(), cvScalar(0), CV_FILLED);
    cnt = 0;
  }
  double fps = 10.0;
...
void MotionFilter::countPixels(cv::Mat const& image)
{
  ...
  for (int i = 0; i < height; i++)
  {
    for (int j = 0; j < width; j++)
    {
      ...
      {
        if (data[i * step + j * channels + k] == 255)
          nonwhite = 1;
        else
        {
          nonwhite = 0;
