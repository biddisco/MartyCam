...
struct TimeValue
{
  ...
  friend std::ostream& operator<<(std::ostream& os, TimeValue const& tv)
  {
...
class Mavg : public boost::noncopyable
{
  ...
  double operator()(TimeValue const& tick);
...
struct cross_alert
{
  ...
  cross_alert(DecayFilter* f)
    : filter(f)
  {
    std::cout << "Filter constructor, f=" << f << std::endl;
  };
  cross_alert(cross_alert const& that)
    : filter(that.filter)
  {
  ...
  bool operator()(bool const is_golden_cross);
...
class DecayFilter
{
  ...
  DecayFilter();
  //
  virtual void process(TimeValue const& tv);
  //
  ...
  streamulus::InputStream<TimeValue> ts;
