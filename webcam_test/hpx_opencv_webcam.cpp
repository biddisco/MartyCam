...
cv::Mat load_image(std::string const& path)
{
  ...
  if (!image.data) throw std::runtime_error("No image data \n");
...
int detect_face(cv::Mat& img, cv::CascadeClassifier& cascade, cv::CascadeClassifier& nestedCascade,
    double scale)
{
  ...
  for (size_t i = 0; i < faces.size(); i++)
  {
    ...
    nestedCascade.detectMultiScale(
        smallImgROI, nestedObjects, 1.1, 2, 0 | cv::CASCADE_SCALE_IMAGE, cv::Size(30, 30));
...
int start_webcam_capture(int num_frames, cv::CascadeClassifier cascade,
    cv::CascadeClassifier nestedCascade, double scale, hpx::execution::parallel_executor def_exec)
{
  ...
  std::string curr_pool_name = hpx::threads::get_pool(hpx::threads::get_self_id())->get_pool_name();
  ...
  while (true)
  {
    ...
    if (captured_frame.empty()) break;

    processed_frame = captured_frame.clone();

    hpx::future<int> detection_flag_f = hpx::async(def_exec, detect_face, std::ref(processed_frame),
    ...
    if (detection_flag == 0)
    {
      cv::imshow("Face Recognition from " + curr_pool_name, processed_frame);
...
void show_image(cv::Mat const& image, std::string win_name)
{
...
void save_image(cv::Mat const& image, std::string const& path)
{
...
int hpx_main(hpx::program_options::variables_map& vm)
{
  ...
  hpx::execution::parallel_executor opencv_executor(
      &hpx::resource::get_thread_pool(opencv_tp_name));
  std::cout << "[hpx_main] Created default and " << opencv_tp_name << " pool_executors \n";
  ...
  std::string nestedCascadePath =
      DATA_PATH + std::string("/models/haarcascade_eye_tree_eyeglasses.xml");
  std::string cascadePath = DATA_PATH + std::string("/models/haarcascade_frontalface_default.xml");

  if (detect_eyes) nestedCascade.load(nestedCascadePath);
  cascade.load(cascadePath);

  hpx::future<int> webcam_closed = hpx::async<>(opencv_executor, &start_webcam_capture, num_frames,
      cascade, nestedCascade, scale, def_executor);

  cv::Mat webcam_img;
  cv::Mat grey_webcam_img;
  // schedule taking webcam image on the opencv pool
  hpx::future<cv::Mat> f_image = webcam_closed.then([&](hpx::future<int>&& wc) {
    std::cout << "Webcam closed\n";
    return take_webcam_image();
  });

  // schedule transforming webcam image to grey-scale on opencv pool
  hpx::future<cv::Mat> f_grey_image = f_image.then([&](hpx::future<cv::Mat>&& fi) {
    ...
    hpx::resource::partitioner& rp, hpx::program_options::variables_map const&)
{
  ...
  rp.create_thread_pool(opencv_tp_name, hpx::resource::scheduling_policy::local_priority_fifo);

  std::cout << "[main] " << "thread_pool " << opencv_tp_name << " created\n";
  ...
  for (hpx::resource::numa_domain const& d : rp.numa_domains())
  {
    for (hpx::resource::core const& c : d.cores())
    {
      for (hpx::resource::pu const& p : c.pus())
      {
        ...
        {
          std::cout << "[main] Added pu " << count++ << " to " << opencv_tp_name << "thread pool\n";
...
int main(int argc, char* argv[])
{
  ...
  desc_cmdline.add_options()("opencv_tp_num_threads,m", po::value<int>()->default_value(1),
      "Number of threads to assign to custom pool")("num_frames,f",
      po::value<int>()->default_value(25),
      "Number of frames per second in the video stream.")("detect_eyes,e",
      po::value<bool>()->default_value(false), "If set to true the application will detect eyes.");
