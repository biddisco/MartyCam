...
int main(int argc, char *argv[]) {
  ...
  if (const char *env = std::getenv("MARTYCAM_HEADLESS_TEST")) {
  ...
  if (!headlessMode) {
    try {
      ...
      cout << "Display backend is unavailable, switching to headless mode: "
           << ex.what() << endl;
      headlessMode = true;
    }
  }
  ...
  int framesRead = 0;
  const int maxHeadlessFrames = 100;
  ...
  while (true) {
    ...
    if (!headlessMode) {
      ...
      if (framesRead >= maxHeadlessFrames) {
        cout << "Headless mode: captured " << framesRead
             << " frames successfully" << endl;
        break;
      }
    }
  }
