...
SettingsWidget::SettingsWidget(QWidget *parent) : QWidget(parent) {
  ...
  setMinimumWidth(150);
  //
  connect(ui.threshold, SIGNAL(valueChanged(int)), this,
          SLOT(onThresholdChanged(int)));
  connect(ui.average, SIGNAL(valueChanged(int)), this,
          SLOT(onAverageChanged(int)));
  connect(ui.erode, SIGNAL(valueChanged(int)), this, SLOT(onErodeChanged(int)));
  connect(ui.dilate, SIGNAL(valueChanged(int)), this,
          SLOT(onDilateChanged(int)));
  ...
  connect(ui.WriteMotionAVI, SIGNAL(toggled(bool)), this,
          SLOT(onWriteMotionAVIToggled(bool)));
  connect(&this->clock, SIGNAL(timeout()), this, SLOT(onTimer()));
  connect(ui.blendRatio, SIGNAL(valueChanged(int)), this,
          SLOT(onBlendChanged(int)));
  connect(ui.noiseBlend, SIGNAL(valueChanged(int)), this,
          SLOT(onBlendChanged(int)));
  //
  ...
  connect(ui.eyesRecog_CheckBox, SIGNAL(stateChanged(int)), this,
          SLOT(onEyesRecogStateChanged(int)));
  ...
  connect(ui.startTimeLapse, SIGNAL(clicked()), this,
          SLOT(onStartTimeLapseClicked()));
...
//----------------------------------------------------------------------------
void SettingsWidget::setThreads(CaptureThread_SP capthread,
                                ProcessingThread_SP procthread) {
...
void SettingsWidget::onAddCameraClicked() {
  ...
  QMessageBox::warning(
      this, tr("Not implemented warning."),
...
void SettingsWidget::setupCameraList() {
  disconnect(this->ui.cameraSelect, SIGNAL(currentIndexChanged(int)), this,
             SLOT(onCameraSelection(int)));
  ...
  for (stringpairlist::iterator it = cameras.begin(); it != cameras.end();
       ++it) {
    this->ui.cameraSelect->addItem(it->first.c_str());
  }
  this->ui.cameraSelect->blockSignals(false);
  connect(this->ui.cameraSelect, SIGNAL(currentIndexChanged(int)), this,
          SLOT(onCameraSelection(int)));
...
void SettingsWidget::onTimer() {
  ...
  int s = el - h * 3600000 - m * 60000 > 0
              ? (el - (h * 3600000 + m * 60000)) / 1000
              : 0;
  int ms = el - (h * 3600000 + m * 60000 + s * 1000) > 0
               ? el - (h * 3600000 + m * 60000 + s * 1000)
               : el;
  if (ms > 500)
    s += 1;
...
MotionFilterParams SettingsWidget::getMotionFilterParams() {
  ...
  motionFilterParams.blendRatio =
      static_cast<float>(ui.blendRatio->value()) / 100;
...
FaceRecogFilterParams SettingsWidget::getFaceRecogFilterParams() {
  ...
  faceRecogFilterParams.decimationCoeff =
      ui.decimationCoeff_HorizontalSlider->value();
...
void SettingsWidget::SetupAVIStrings() {
  ...
  QString fileName2 = "TimeLapse" + QDateTime::currentDateTime().toString(
                                        "yyyy-MM-dd_hh-mm-ss");
  this->capturethread->setWriteTimeLapseAVIName(
      fileName2.toLatin1().constData());
}
//----------------------------------------------------------------------------
void SettingsWidget::RecordMotionAVI(bool state) {
  ...
  this->AVI_EndTime = QTime::currentTime()
                          .addSecs(secs + mins * 60 + hrs * 60 * 60)
                          .addMSecs(500);
  //
  if (!state) {
...
void SettingsWidget::onBlendChanged(int value) {
  ...
  this->processingthread->setBlendRatios(this->ui.blendRatio->value() / 100.0,
                                         this->ui.noiseBlend->value() / 100.0);
}
//----------------------------------------------------------------------------
...
void SettingsWidget::saveSettings() {
  QString settingsFileName =
      QCoreApplication::applicationDirPath() + "/MartyCam.ini";
  ...
  settings.setValue("eyesRecognition",
                    this->ui.eyesRecog_CheckBox->checkState());
  settings.setValue("decimationCoeff",
                    this->ui.decimationCoeff_HorizontalSlider->value());
  settings.endGroup();
  ...
  settings.setValue("requestedFps",
                    this->ui.requestedFps_HorizontalSlider->value());
...
void SettingsWidget::loadSettings() {
  ...
  //
  settings.beginGroup("MotionDetection");
  SilentCall(this->ui.threshold)
      ->setValue(settings.value("threshold", 3).toInt());
  SilentCall(this->ui.t_label)
      ->setText(settings.value("threshold", 3).toString());
  SilentCall(this->ui.average)->setValue(settings.value("average", 10).toInt());
  SilentCall(this->ui.a_label)
      ->setText(
          QString::number((settings.value("average", 10).toFloat() / 100)));
  ...
  SilentCall(&this->ImageButtonGroup)
      ->button(settings.value("display", 0).toInt())
      ->click();
  SilentCall(this->ui.blendRatio)
      ->setValue(settings.value("blendImage", 0.5).toInt());
  SilentCall(this->ui.noiseBlend)
      ->setValue(settings.value("blendNoise", 0.5).toInt());
  settings.endGroup();

  settings.beginGroup("FaceRecognition");
  SilentCall(this->ui.eyesRecog_CheckBox)
      ->setChecked(settings.value("eyesRecognition", false).toBool());
  SilentCall(this->ui.decimationCoeff_HorizontalSlider)
      ->setValue(settings.value("decimationCoeff", 50).toInt());
  SilentCall(this->ui.decimationCoeff_Label)
      ->setText(decimationCoeffToQString(
          (settings.value("decimationCoeff", 50).toInt())));
  settings.endGroup();

  settings.beginGroup("GeneralSettings");
  SilentCall(this->ui.requestedFps_HorizontalSlider)
  ...
  SilentCall(&this->ResolutionButtonGroup)
      ...
      ->click();
  SilentCall(&this->RotateButtonGroup)
      ->button(settings.value("rotation", 0).toInt())
      ->click();
  SilentCall(this->ui.cameraSelect)
      ->setCurrentIndex(settings.value("cameraIndex", 0).toInt());
  SilentCall(this->ui.tabWidget)
      ->setCurrentIndex(settings.value("processingType", 0).toInt());
  //
  settings.endGroup();

  settings.beginGroup("SavingStream");
  SilentCall(this->ui.avi_directory)
      ->setText(settings.value("aviDirectory", "$HOME/wildlife").toString());
  this->SnapshotId = settings.value("snapshot", 0).toInt();
  SilentCall(this->ui.AVI_Duration)
      ->setTime(settings.value("aviDuration", QTime(0, 0, 10)).toTime());
  settings.endGroup();

  settings.beginGroup("TimeLapse");
  SilentCall(this->ui.startDateTime)
      ->setDateTime(settings
                        .value("startDateTime", QDateTime(QDate::currentDate(),
                                                          QTime::currentTime()))
                        .toDateTime());
  SilentCall(this->ui.interval)
      ->setTime(settings.value("interval", QTime(0, 1, 00)).toTime());
  SilentCall(this->ui.duration)
      ->setTime(
          settings.value("duration", QDateTime(QDate(0, 0, 1), QTime(0, 1, 0)))
              .toTime());
  settings.endGroup();
}
//----------------------------------------------------------------------------
...
void SettingsWidget::onTabChanged(int currentTabIndex) {
  ...
  case 1:
    ...
    break;
  default:
    std::cout << "Current tab index = " << std::to_string(currentTabIndex)
              << " is unsupported\n";
    break;
  }
}
...
QString SettingsWidget::decimationCoeffToQString(int sliderVal) {
  if (sliderVal > 100 || sliderVal < 0) {
    // return QString();
    QMessageBox::warning(
        this, tr("Unsupported decimation Coeff Value."),
        tr(qPrintable("Current Decimation Coeff = " +
                      QString::number(sliderVal) + "is unsupported\n")));
    return "?";
