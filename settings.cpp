...
void SettingsWidget::loadSettings()
{
  ...
  SilentCall(this->ui.a_label)
      ->setText(QString::number((settings.value("average", 10).toFloat() / 100)));
  ...
  SilentCall(this->ui.decimationCoeff_Label)
      ->setText(decimationCoeffToQString((settings.value("decimationCoeff", 50).toInt())));
  settings.endGroup();
...
QString SettingsWidget::decimationCoeffToQString(int sliderVal)
{
  ...
  {
    // return QString();
    QMessageBox::warning(this, tr("Unsupported decimation Coeff Value."),
        tr(qPrintable(
            "Current Decimation Coeff = " + QString::number(sliderVal) + "is unsupported\n")));
    return "?";
  }
