...
class unique
{
  ...
  boost::optional<T> operator()(T const& value)
  {
...
struct print
{
  ...
  template <typename T>
  typename result<print(T)>::type operator()(T const& value) const
  {
...
struct thisptr
{
  ...
  template <typename T>
  typename result<thisptr(T)>::type operator()(T const* ptr) const
  {
    return ptr;
  }
};
...
//----------------------------------------------------------------------------
bool cross_alert::operator()(bool const is_golden_cross)
{
  if (is_golden_cross)
  {
