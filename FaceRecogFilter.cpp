...
void FaceRecogFilter::process(cv::Mat const& img)
{
  ...
  cv::putText(img, timestring.toLatin1().data(),
      cvPoint(img.size().width - text_size.width - 4, text_size.height + 4), CV_FONT_HERSHEY_PLAIN,
      1.0, cv::Scalar(255, 255, 255, 0), 1);
  //
  // Pass final image to GUI
  //
  if (renderer) { renderer->process(img); }
}
