...
/* 
  ...
  @TODO Zanim to zrobisz - przemy�l gruntownie mechanizm kana��w ... 
   
2. Zmieni� nieco interfejs chart�w ... stworzy� tzw. grupy kana��w ... 
       oraz doda� mo�liwo�� wy�wietlania wybranych kana�ow z wykresu 1 na wykresie 2. 
       Przy okazji mo�na by by�o podmieni� w�a�ciwo�ci tych kana��w ... np kolor itp. 
       Ka�dy kana� identyfikowany by� by odatkowo przez swoj� nazw� ... 
       Doda� kontekstowe menu z standardowymi opcjami dla Chart'a oraz list� grup kana��w 
       co pozwoli�o by zarz�dza� grup� kana��w oraz ka�dym kana�em w grupie osobno. 
       
       Pomys� na implementacj�: 
       Je�eli kana� nie ma nazwy nie jest wy�wietlany w Legendzie ... 
       Interfejs kana��w pozostaje taki sam ... dochodz� tylko nowe metody ktore mog� zwr�ci� 
       kana�y w nowej grupie oraz pozwala to na tworzenie nowych kana��w w nowych grupach :) 
       
       np . addChannel("Nazwa grupy kana��w", Channel(...)); 
       oraz addChannel(nr_grupy,Channel(...)); 
            addChannel(Channel(...)); // dodaje channel do podstawowej grupy 
            
            channels(); zwraca referencj� do kana��w we wszystkich grupach
            channels(nr_grupy) ... zwraca referencj� do kana�ow konkretnej grupy :D 
            channels("Nazwa grupy") ... 
                        
           pokazanie grupy kana��w z chart'a 1 na charcie 2. to 
             
           chart1->addGroup("new group",chart2->channles("Wskazana grupa"); 
           potem mo�na ju� r�cznie sobie dokonfigurowa� reszt� ... np. 
           chart1->channels("Wskazana grupa")[1].isVisible(false); 
           lub chart1->channels("Wskazana grupa").remove("Pozycja tloka"); 
           
           
             
    Pomys� 2 zdecydowanie bardziej mi si� podoba ... jest bardziej uniwersalny 
    mo�na by go zastosowa� te� w innych aplikacjach ... 
    
    Na razie zrobi� na sztywno ... dodawane kana�y po prostu jak do tej pory. 
...
{
  m_painterDecorator.reset(new Standard::ScalesGridDecorator(new Standard::ChannelDecorator(
...
// position
int Chart::position() const { return static_cast<int>(m_scalegrid.pos); }
...
void Chart::setPosition(int i) { setPosition(static_cast<double>(i)); }

// size
double Chart::size() const { return m_xsize; }
...
void Chart::setSize(int i) { setSize(static_cast<double>(i)); }

// ***** channelMinimum
double Chart::channelMinimum() const
{
  if (m_channels.size()) return m_channels[m_channel].minimum();
  return 0.0;
}
...
{
  if (m_channels.size()) return m_channels[m_channel].maximum();
...
// ***** channel
int Chart::channel() const { return m_channel; }
...
QColor Chart::channelColor() const
{
  ...
  if (m_channels.size()) C = m_channels[m_channel].pen().color();
...
{
  if (m_channels.size()) return m_channels[m_channel].showScale();
  return false;
}

bool Chart::channelVisible() const
{
  if (m_channels.size()) return m_channels[m_channel].visible();
  return false;
}
...
{
  QString name;
  if (m_channels.size()) name = m_channels[m_channel].name();
  return name;
}
...
QColor Chart::topBackgroundColor() const { return m_topBackgroundColor; }

QColor Chart::bottomBackgroundColor() const { return m_bottomBackgroundColor; }

QColor Chart::gridColor() const { return m_gridColor; }
...
// ***** zoom
double Chart::zoom() const { return m_zoom; }
...
// Zas�ania setFont QWidget by mo�na by�o od�wie�y� ca�y komponent
...
void Chart::setTopBackgroundColor(QColor const& c)
{
  m_topBackgroundColor = c;
  updateWithBackground();
}

void Chart::setBottomBackgrounColor(QColor const& c)
{
  m_bottomBackgroundColor = c;
  updateWithBackground();
}

void Chart::setGridColor(QColor const& c)
{
  m_gridColor = c;
  updateWithBackground();
}

// Ustawia koniec malowania i nast�puje malownie z antialiasingiem
bool Chart::isPaintOver() const { return m_isPaintOver; }

void Chart::contextMenuEvent(QContextMenuEvent* e) { contextMenu()->exec(e->globalPos()); }
...
QMenu* Chart::contextMenu()
{
  ...
  while (i != m_channels.end())
  {
    ...
    // Tutaj czy kana� ma by� pokazywany w og�le
...
/** Maluje pozycj� kursora na obiekcie danych */
void Chart::paintCursorPosition(QPainter& painter)
{
  ...
  {
    painter.drawLine(QPoint(m_currentCurPoint.x(), m_clip.y()),
