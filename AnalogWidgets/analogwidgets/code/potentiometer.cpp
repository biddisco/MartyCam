...
int PotentioMeter::rotationRange() const { return m_rotationRange; }

QString PotentioMeter::svgResourceFile() const { return m_filePath; }
/*---------------------------------------------------------------------------
                                PUBLIC SLOTS
---------------------------------------------------------------------------*/
...
void PotentioMeter::setSvgResourceFile(const QString &filePath) {
  ...
  if (!m_renderer->load(filePath)) {
    qDebug("PotentioMeter::setSvgResourceFile can't load file %s",
           qPrintable(m_filePath));
    m_filePath = ":/default/resources/dial1.svg";
    m_renderer->load(m_filePath);
  } else
    qDebug("PotentioMeter::setSvgResourceFile %s loaded",
           qPrintable(m_filePath));
...
void PotentioMeter::paintEvent(QPaintEvent *) {
  ...
  m_renderer->render(&painter, QString("foreground"),
                     frame); //,QString("background"),frame);
...
void PotentioMeter::mouseMoveEvent(QMouseEvent *event) {
  if (event->buttons() & Qt::LeftButton) {
    ...
    if (value() % rRange - m_inc != 0 && m_inc != 0) {
      ...
      // qDebug("m_inc %d  rRange %d, value %d, diff %d, v %d, index
      // %d",m_inc,rRange,index*rRange + m_inc,diff, v, index); qDebug("Value
      // %d",value());
