...
TestWidget::TestWidget(QMainWindow *parent) : QMainWindow(parent) {
  initialize();
  connections();
}
...
void TestWidget::initialize() {
  ...
  // za�adowanie listy wtrysk�w do CobmboBox'a
  QString cwd = WTR_DIR;
  //        QDir dir( cwd + "/wtr");
  QDir dir("C:\\Code\\martycam\\AnalogWidgets\\test\\wtr");
  ...
  connect(comboBox, SIGNAL(activated(int)), this,
          SLOT(ComboBoxChoiceChanged(int)));
...
void TestWidget::initCharts() {
  ...
  Channel cisnienie3(0, 350,
                     new DoubleDataContainer<vint, ldouble>(times, press3),
                     tr(CALCPRESS), vPen);
...
void TestWidget::movePosition(int val) { chart->setPosition(val); }
void TestWidget::setSize(int val) { chart->setSize(val); }
...
void TestWidget::ComboBoxChoiceChanged(int index) {
  ...
  switch (index) {
  case 0:
    val = static_cast<int>(meter->value());
    break;
  case 1:
    val = static_cast<int>(meter->minimum());
    break;
  case 2:
    val = static_cast<int>(meter->maximum());
    break;
  case 3:
    val = static_cast<int>(meter->nominal());
    break;
  case 4:
    val = static_cast<int>(meter->critical());
    break;
  case 5:
    val = static_cast<int>(meter->digitOffset());
    break;
  case 6:
    val = static_cast<int>(meter->valueOffset());
    break;
  }
...
void TestWidget::SpinBoxValueChanged(int val) {
  ...
  case 0:
    ...
    break;
  case 1:
    meter->setMinimum(val);
    break;
  case 2:
    meter->setMaximum(val);
    break;
  case 3:
    meter->setNominal(val);
    break;
  case 4:
    meter->setCritical(val);
    break;
  case 5:
    meter->setDigitOffset(val);
    break;
  case 6:
    meter->setValueOffset(val);
    break;
  }
}
...
void TestWidget::loadInjection(const QString &file) {
  ...
  char buffer[1024];
  std::string f("C:\\Code\\martycam\\AnalogWidgets\\test\\wtr\\");
  ...
  */
  ...
  while (!filein.eof()) {
    ...
    // filein>>buffer;
    //  cout<<time<<"  p:"<<pos<<" v:"<<vel<<" p1:"<<p1<<endl; //" "<<p2<<"
    //  "<<p3<<endl;
    ++time;
  }
...
void TestWidget::closeEvent(QCloseEvent * /* event */) { qApp->quit(); }
