...
ScriptTool::ScriptTool(QWidget *parent) : QWidget(parent) { init(); }
...
void ScriptTool::setPath() {
  setPath(QFileDialog::getOpenFileName(this, trUtf8("Open file"), QString(),
                                       trUtf8("(Qt Script *.qjs)")));
}
...
void ScriptTool::setPath(const QString &path) { fileEdit->setText(path); }
...
void ScriptTool::init() {
  ...
  connect(PCombo, SIGNAL(textHighlighted(const QString &)), this,
          SLOT(updatePValue(const QString &)));
  resizeEvent(NULL);
