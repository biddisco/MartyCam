...
MartyCam::MartyCam(const hpx::execution::parallel_executor &defaultExec,
  ...
  this->ui.gridLayout->addWidget(this->renderWidget.get(), 0,
                                 Qt::AlignHCenter || Qt::AlignTop);

  this->imageBuffer =
      ImageBuffer(new ConcurrentCircularBuffer<cv::Mat>(IMAGE_BUFF_CAPACITY));

  //
  connect(this->renderWidget.get(), SIGNAL(mouseDblClicked(const QPoint &)),
          this, SLOT(onMouseDoubleClickEvent(const QPoint &)));
  //
  ...
  connect(this->settingsWidget.get(), SIGNAL(resolutionSelected(cv::Size)),
          this, SLOT(onResolutionSelected(cv::Size)));
  connect(this->settingsWidget.get(), SIGNAL(rotationChanged(int)), this,
          SLOT(onRotationChanged(int)));
  connect(this->settingsWidget.get(), SIGNAL(CameraIndexChanged(int, QString)),
          this, SLOT(onCameraIndexChanged(int, QString)));
  ...
  // separate.
  //
  connect(this->ui.user_trackval, SIGNAL(valueChanged(int)), this,
          SLOT(onUserTrackChanged(int)));
  //
  //
  this->loadSettings();
  ...
  while (!captureLaunched && this->cameraIndex >= 0) {
    ...
    while (!requestedSizeCorrect && numeResolutionsToTry > 0) {
      ...
      this->createCaptureThread(res, this->cameraIndex, camerastring,
                                blockingExecutor);
...
//----------------------------------------------------------------------------
void MartyCam::createCaptureThread(cv::Size &size, int camera,
                                   const std::string &cameraname,
                                   hpx::execution::parallel_executor exec) {
  this->captureThread = std::make_shared<CaptureThread>(
      imageBuffer, size, this->settingsWidget->getSelectedRotation(), camera,
      cameraname, this->blockingExecutor,
...
void MartyCam::createProcessingThread(ProcessingThread *oldThread,
  ...
  this->processingThread = std::make_shared<ProcessingThread>(
      imageBuffer, exec, processingType, mfp, frfp);
  if (oldThread)
    this->processingThread->CopySettings(oldThread);
  this->processingThread->setRootFilter(renderWidget.get());
  this->processingThread->startProcessing();

  this->settingsWidget->setThreads(this->captureThread, this->processingThread);
  //
  connect(this->processingThread.get(), SIGNAL(NewData()), this,
          SLOT(updateGUI()), Qt::QueuedConnection);
}
//----------------------------------------------------------------------------
...
void MartyCam::onResolutionSelected(cv::Size newSize) {
  if (this->captureThread->setResolution(newSize))
    ...
    QMessageBox::warning(
        this, tr("Unsupported Resolution."),
...
void MartyCam::updateGUI() {
  ...
  this->ui.detect_value->setText(QString("%1").arg(
      this->processingThread->motionFilter->motionEstimate, 4, 'f', 2));
  ...
  if (!this->captureThread->TimeLapseAVI_Writing) {
    if (this->settingsWidget->TimeLapseEnabled()) {
      if (now > start && now < stop) {
        ...
        this->captureThread->startTimeLapse(
...
void MartyCam::onRecordingStateChanged(bool state) {
  if (state) {
    this->ui.RecordingEnabled->setStyleSheet(
    ...
    this->ui.RecordingEnabled->setStyleSheet(
        "QCheckBox { background-color: window; }");
  }
}
//----------------------------------------------------------------------------
void MartyCam::resetChart() { this->ui.chart->channels().clear(); }
//----------------------------------------------------------------------------
